...
func TestParseRAPrefixes(t *testing.T) {
	// RA header: type(1) + code(1) + checksum(2) + hop(1) + flags(1) + lifetime(2) + reachable(4) + retrans(4) = 16 bytes
	raHeader := make([]byte, 16)
	raHeader[0] = icmpv6TypeRA

	// Prefix Information option (32 bytes, type=3, len=4):
	//   [0]  type=3
	//   [1]  len=4 (4×8=32)
	//   [2]  prefix length=64
	//   [3]  flags
	...
	prefixOpt[0] = ndpOptionPrefixInfo
	prefixOpt[1] = 4
	prefixOpt[2] = 64
	prefixOpt[3] = 0xc0
	prefixOpt[4] = 0xff; prefixOpt[5] = 0xff; prefixOpt[6] = 0xff; prefixOpt[7] = 0xff
	prefixOpt[8] = 0xff; prefixOpt[9] = 0xff; prefixOpt[10] = 0xff; prefixOpt[11] = 0xff
	copy(prefixOpt[16:], net.ParseIP("fd36:1fa8:0d5a::").To16())
	...
	if prefixes[0] != "fd36:1fa8:d5a::/64" {
		t.Errorf("Expected fd36:1fa8:d5a::/64, got %s", prefixes[0])
	}

	t.Run("Non-ULA prefix is ignored", func(t *testing.T) {
		prefixOpt2 := make([]byte, 32)
